...
class Auth_model extends Model {
    ...
    public function create() {
        ...
        $admissionslotInfo = $this->db->query('SELECT * from admission_slot_panels where campus_id='.$this->_campus.' and class_id='.$this->_adClass.' and status=1 AND panel_id IN (SELECT panel_id FROM admission_panels WHERE phase_id IN (SELECT phase_id FROM admission_phases WHERE ph_id IN (SELECT ph_id FROM phases WHERE STATUS =1) )) AND capacity > 0 order by panel_id,slot_id ASC')->row();

        $admissionPhaseInfo = $this->db->query('SELECT * from admission_phases where ph_id IN(select ph_id from phases WHERE status=1) and campus_id='.$this->_campus.' and class_id='.$this->_adClass.' and status=1')->row();

        $last_row = $this->db->query('select * from admission_students where campus_id='.$this->_campus.' AND gender="'.$this->_gender.'" AND ad_student_id IN(select ad_std_id from admission_registration where class_id='.$this->_adClass.') ORDER BY ad_student_id DESC')->row();
