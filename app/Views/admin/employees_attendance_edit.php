...
<?php
	if(isset($info) ){
		...
		$header = 'Edit Employees Attendance';
		$id = $info->student_id;
		...
		$header = 'Add Employees Attendance';
		$id = '';
		$class_id = '';
		$subject_id = '';
		$obtained_marks = 0;
    ...
    <div class="col-lg-12">
      <div class="card card-primary card-outline card-tabs">
        ...
        <div class="tab-content">
		 <?php
			echo form_open(base_url('admin/employees_attendance/save'), 'role="form" id="user-edit-form"');
		 ...
		 <div class="row">
		   ...
		   <input type="hidden" name="campus_id" id="campus_id" value="<?php echo $campus_id; ?>" />
		    <div class="form-inline col-lg-6 col-lg-offset-3">
	            ...
	            <div class="form-group  pull-left" >
	            <button type="button" onclick="get_employees();" class="btn btn-sm btn-primary" style="height: 24px;line-height: 10px;">View</button>
	           </div>
	          </div> 
		 </div>
		 <br><br>
...
function get_employees() {
		...
		var date = $('#date').val();		
 	      $.ajax({
            url: '/admin/employees_attendance/get_employees',
            type: "POST",
            data:{campus_id:campus_id,date:date },
	...
	$('#user-edit-form').ajaxForm({
		...
		success:function(responseText, statusText, xhr, form){
			...
			if(json.success){
				...
				if($id == ''){
					?>
					//location.href = '#/students_attendance?m=add';
					<?php
				}else{
					?>
					//location.href = '#/students_attendance?m=add';
