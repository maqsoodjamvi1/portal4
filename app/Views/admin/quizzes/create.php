...
<section class="content">
  ...
  <label class="d-block">Adaptive Quiz</label>
  <div class="custom-control custom-switch">
    <input type="checkbox"
           ...
           name="is_adaptive"
           value="1">
    <label class="custom-control-label" for="is_adaptive">
      Enable Levels & AI Progression
    </label>
  </div>
</div>


<div class="card mb-3 border-0 shadow-sm d-none" id="adaptiveLevelsCard">
  <div class="card-header bg-light py-2">
    <h3 class="card-title mb-0">Quiz Levels</h3>
    <small class="text-muted">
      Define levels and assign questions to each level.
    </small>
  </div>
...
$qbNestedWrap.off('show.bs.collapse', '.qb-subject-card .collapse')
  ...
  function renderQuestions(rows) {
    ...
    $qbMaster.prop('checked', false);
    applyExistingAssignments();
  }



function applyExistingAssignments() {
  existingAssigned.forEach(row => {
    assignedQuestions[row.question_id] = row.level_no;
  });

  $('.qb-check').each(function () {
    const qid = $(this).val();
    if (assignedQuestions[qid]) {
      $(this)
        .prop('disabled', true)
        .closest('tr')
        .addClass('qb-assigned');
    }
  });
}
...
<script>
/* ===============================
   ADAPTIVE QUIZ GLOBAL STATE
   =============================== */

// question_id => level_no
const assignedQuestions = {};

// optional: for edit mode
const existingAssigned = <?= json_encode($assignedMap ?? []) ?>;

  // expose to other scripts
  window.qbReloadFromTypeFilters = reloadQuestionsFromSelected;

$('#is_adaptive').on('change', function () {
  if ($(this).is(':checked')) {
    $('#adaptiveLevelsCard').removeClass('d-none');
  } else {
    $('#adaptiveLevelsCard').addClass('d-none');
  }
});
...
function addLevel() {
  ...
  const html = `
    <div class="border rounded p-3 mb-3 level-card"
         data-level-no="${levelCounter}">
      <h6>Level ${levelCounter}</h6>
      ...
      <div class="form-row">
        <div class="form-group col-md-3">
          ...
          <input type="number"
                 class="form-control"
                 name="levels[${levelCounter}][passing_percentage]"
        ...
        <div class="form-group col-md-3">
          ...
          <select class="form-control"
                  name="levels[${levelCounter}][base_difficulty]">
      ...
      <input type="hidden"
             name="levels[${levelCounter}][question_ids]"
             class="level-question-ids">

      <small class="text-muted">
        Select questions below, then click “Assign to Level ${levelCounter}”
      </small>

      <button type="button"
              class="btn btn-xs btn-outline-secondary mt-1 assignLevelBtn"
              data-level="${levelCounter}">
        Assign selected questions to this level
      </button>

      <div class="mt-2 assignedQuestionsText text-muted small"></div>
      <button type="button"
        class="btn btn-xs btn-outline-danger mt-1 removeLevelBtn"
        data-level="<?= $levelNo ?? '' ?>">
  Clear this level
</button>
  ...
  $('#levelsWrap').append(html);
}


$('#addLevelBtn').on('click', addLevel);

$(document).on('click', '.assignLevelBtn', function () {

  const levelNo = $(this).data('level');
  const $level  = $('.level-card[data-level-no="' + levelNo + '"]');

  const ids = [];
  $('.qb-check:checked').each(function () {
    ids.push($(this).val());
  });

  if (!ids.length) {
    alert('Select questions before assigning.');
    return;
  }

  // ❌ Prevent duplicates
  for (const id of ids) {
    if (assignedQuestions[id]) {
      alert(
        'Question #' + id +
        ' is already assigned to Level ' +
  ...
  // ✅ Assign
  ids.forEach(id => {
    assignedQuestions[id] = levelNo;
  });

  // Save snapshot
  $level.find('.level-question-ids').val(ids.join(','));

  // UI summary
  $level.find('.assignedQuestionsText')
    .html('<strong>' + ids.length + '</strong> questions locked.');

  // 🔒 VISUAL LOCK
  ids.forEach(id => {
    const $row = $('.qb-check[value="' + id + '"]').closest('tr');
...
$(document).on('click', '.removeLevelBtn', function () {
  ...
  const qids = ($level.find('.level-question-ids').val() || '').split(',');

  qids.forEach(id => {
    delete assignedQuestions[id];
    const $row = $('.qb-check[value="' + id + '"]').closest('tr');
    $row.removeClass('qb-assigned');
    $row.find('.qb-check').prop('disabled', false);
  });

  $level.find('.level-question-ids').val('');
  $level.find('.assignedQuestionsText').html('');
});
