...
/* Urdu text alignment */
.urdu-text {
    font-family: 'Jameel Noori Nastaleeq', 'Jameel Urdu Nastaleeq', serif;
    font-size: 18pt;
    text-align: right;
    direction: rtl;
    line-height: 1.8;
    ...
    <div class="p-1">
        ...
        <div class="question-header">
            <div class="d-flex justify-content-between align-items-start">
                <div style="flex: 1;">
                    <strong style="font-size: 1.5em;">Q<?= $globalQNo++ ?>.</strong>
                    <span class="question-text <?= !empty($q->question) && preg_match('/[\x{0600}-\x{06FF}]/u', $q->question) ? 'urdu-text' : '' ?>">
                        <?= esc($q->question) ?>
                    </span>
        ...
        <ul class="q-options-row">
            <?php foreach (['A','B','C','D'] as $opt):
                ...
                if (!empty($q->$field)): 
                    $isUrdu = !empty($q->$field) && preg_match('/[\x{0600}-\x{06FF}]/u', $q->$field);
            ?>
            <li class="q-option <?= ($correct === $opt) ? 'is-correct' : '' ?> <?= $isUrdu ? 'urdu-text' : '' ?>">
        ...
        <ul class="q-options-row">
            <?php foreach (['A','B','C','D'] as $opt):
                ...
                if (!empty($q->$field)): 
                    ...
                    $isUrdu = !empty($q->$field) && preg_match('/[\x{0600}-\x{06FF}]/u', $q->$field);
            ?>
            <li class="q-option <?= $isCorrect ? 'is-correct' : '' ?> <?= $isUrdu ? 'urdu-text' : '' ?>">
                <strong><?= $opt ?>.</strong> <?= esc($q->$field) ?>
            </li>
            <?php endif; endforeach; ?>
        </ul>
        ...
        <?php
            ...
            $isUrdu = !empty($questionText) && preg_match('/[\x{0600}-\x{06FF}]/u', $questionText);
        ?>
        <div style="margin: 4px 0;" class="<?= $isUrdu ? 'urdu-text' : '' ?>">
            <?= $displayText ?>
        ...
        <!-- ================= SHORT ANSWER ================= -->
        <?php if ($type === 'short'): ?>
        <div class="solution-box <?= !empty($q->answer_text) && preg_match('/[\x{0600}-\x{06FF}]/u', $q->answer_text) ? 'urdu-text' : '' ?>" 
             style="background: #fef3c7; border-color: #f59e0b; font-size: 0.9em;">
        ...
        <ul class="q-options-row">
            <?php foreach (['A','B','C','D'] as $opt):
                ...
                if (!empty($q->$field)):
                    $isUrdu = !empty($q->$field) && preg_match('/[\x{0600}-\x{06FF}]/u', $q->$field);
            ?>
            <li class="q-option <?= ($mathCorrect === $opt) ? 'is-correct' : '' ?> <?= $isUrdu ? 'urdu-text' : '' ?>">
                <strong><?= $opt ?>.</strong> <?= esc($q->$field) ?>
            </li>
            <?php endif; endforeach; ?>
        </ul>
        ...
        <div class="match-container">
            <div class="match-column">
                ...
                <?php foreach ($leftItems as $index => $left): 
                    $isUrdu = !empty($left) && preg_match('/[\x{0600}-\x{06FF}]/u', $left);
                ?>
                <div class="match-item <?= $isUrdu ? 'urdu-text' : '' ?>">
            ...
            <div class="match-column">
                ...
                <?php foreach ($displayRight as $index => $right): 
                    $isUrdu = !empty($right) && preg_match('/[\x{0600}-\x{06FF}]/u', $right);
                ?>
                <div class="match-item <?= $isUrdu ? 'urdu-text' : '' ?>">
        ...
        <div class="solution-box match-connector-print">
            <?php foreach ($connections as $conn): 
                $leftUrdu = !empty($conn['leftText']) && preg_match('/[\x{0600}-\x{06FF}]/u', $conn['leftText']);
                $rightUrdu = !empty($conn['rightText']) && preg_match('/[\x{0600}-\x{06FF}]/u', $conn['rightText']);
            ?>
            <div style="margin: 2px 0; font-size: 0.9em;">
                <strong><?= $conn['left'] + 1 ?>.</strong> 
                <span class="<?= $leftUrdu ? 'urdu-text' : '' ?>"><?= esc($conn['leftText']) ?></span> 
                ↔ 
                <strong><?= chr(65 + $conn['right']) ?>.</strong> 
                <span class="<?= $rightUrdu ? 'urdu-text' : '' ?>"><?= esc($conn['rightText']) ?></span>
            </div>
            <?php endforeach; ?>
        </div>
        ...
        <?php if (!empty($q->explanation)): 
            $isUrduExp = !empty($q->explanation) && preg_match('/[\x{0600}-\x{06FF}]/u', $q->explanation);
        ?>
        <div class="solution-box <?= $isUrduExp ? 'urdu-text' : '' ?>" style="font-size: 0.9em; margin-top: 4px;">
...
<?php endforeach; ?>
<!-- End of questions for this topic -->
<?php endforeach; ?>
</div>
