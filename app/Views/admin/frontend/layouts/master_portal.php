...
$__isParentPortal = (($__authNav['role'] ?? '') === 'parent');
...
<head>
  ...
  <style>
    ...
    /* Parent portal: allow position:sticky (custom.css overflow-x:hidden on html/body/.wrapper breaks sticky) */
    html.parent-portal-html,
    body.parent-portal-client,
    body.parent-portal-client .wrapper {
      overflow-x: visible !important;
    }
    /* Parent portal: readable language pills in top bar (replaces dashboard row) */
    .parent-topnav-lang {
        display: inline-flex;
        align-items: center;
    ...
    .parent-topnav-lang .btn-lang:hover {
        background: rgba(255, 255, 255, 0.28);
        color: #fff;
    }
    .parent-topnav-lang .btn-lang.is-active {
        background: #fff;
        color: #1d4ed8;
        border-color: #fff;
...
.navbar-nav .dropdown-menu a:hover {
    ...
    /* Parent portal: compact header + mobile bottom bar */
    .parent-portal-client .content-wrapper { padding-bottom: 0; }
    @media (max-width: 767.98px) {
        .parent-portal-client .content-wrapper {
            padding-bottom: calc(68px + env(safe-area-inset-bottom, 0px));
        }
    }
    .parent-portal-bottomnav { display: none; }
    @media (max-width: 767.98px) {
        .parent-portal-client .parent-portal-bottomnav {
            display: flex;
            position: fixed;
            left: 0;
            right: 0;
    ...
    .parent-bottomnav-slot--side {
        flex: 0 0 56px;
        max-width: 56px;
    }
    .parent-portal-bottomnav.parent-bottomnav--double {
        justify-content: center;
        gap: 56px;
        align-items: flex-start;
        padding-left: 20px;
    ...
    .parent-bottomnav-fab:hover {
        color: #fff;
        transform: scale(1.04);
    }
    .parent-bottomnav-fab i { font-size: 1.45rem; }
    .parent-bottomnav-out {
        color: #e5e7eb !important;
        font-size: 1.2rem;
        padding: 0.45rem 0.6rem !important;
        border-radius: 10px;
    }
    .parent-bottomnav-out:hover {
...
<body class="hold-transition sidebar-mini layout-fixed <?= in_array(session('language'), ['ar', 'ur']) ? 'rtl-support' : '' ?> <?= $__isParentPortal ? 'parent-portal-client' : '' ?>">
<div class="wrapper">

  <!-- TOP NAVBAR -->
  <nav class="main-header navbar navbar-expand portal-navbar<?= ! empty($__isParentPortal) ? ' portal-navbar--parent' : '' ?>">
    ...
    <?php if ($__isParentPortal): ?>
    <ul class="navbar-nav ms-auto d-flex flex-row align-items-center flex-nowrap parent-topnav-actions">
      <li class="nav-item d-flex align-items-center me-1 me-sm-2 flex-shrink-1" style="min-width: 0;">
        <div class="parent-topnav-lang" role="group" aria-label="<?= ($__langNav === 'ur') ? 'زبان منتخب کریں' : (($__langNav === 'ar') ? 'اختر اللغة' : 'Choose language') ?>">
          <button type="button" class="btn btn-lang <?= $__langNav === 'en' ? 'is-active' : '' ?>" onclick="changeLanguage('en')">English</button>
          <button type="button" class="btn btn-lang <?= $__langNav === 'ur' ? 'is-active' : '' ?>" onclick="changeLanguage('ur')">اردو</button>
          <button type="button" class="btn btn-lang <?= $__langNav === 'ar' ? 'is-active' : '' ?>" onclick="changeLanguage('ar')">العربية</button>
        </div>
      </li>
      <li class="nav-item flex-shrink-0">
        <a class="btn btn-sm btn-outline-light py-1 px-2" href="<?= route_to('logout') ?>" title="<?= esc($__logoutLabel) ?>">
          <i class="fas fa-sign-out-alt" aria-hidden="true"></i><span class="d-none d-sm-inline"> <?= esc($__logoutLabel) ?></span>
        </a>
      </li>
    </ul>
    <?php else: ?>
...
function toggleLangMenu(event) {
  ...
  <aside class="main-sidebar sidebar-dark-primary elevation-4">
    ...
    <div class="sidebar">
        ...
        $isProfile    = (strpos($path, '/student/profile') === 0);
        ...
        $catalogPath   = '/student/quizzes/all';
        $isPending     = (strpos($path, $pendingPath) === 0);
        $isAttempted   = (strpos($path, $attemptedPath) === 0);
        $isQuizCatalog = (strpos($path, $catalogPath) === 0);
        $isQuizzesRoot = (strpos($path, '/student/quizzes') === 0);
        $isQuizzesTree = $isPending || $isAttempted || $isQuizCatalog || $isQuizzesRoot;
        ?>
        ...
        <nav class="mt-2">
            <ul id="sidebarMenu"
                ...
                <li class="nav-item has-treeview <?= $isQuizzesTree ? 'menu-open' : '' ?>">
                    <a href="<?= base_url('student/quizzes/all') ?>"
                    ...
                    <ul class="nav nav-treeview">
                        <li class="nav-item">
                            <a href="<?= base_url('student/quizzes/all') ?>"
                               class="nav-link <?= $isQuizCatalog ? 'active' : '' ?>">
  ...
  <?php if (! empty($__isParentPortal)): ?>
  <nav class="parent-portal-bottomnav parent-bottomnav--double d-md-none" aria-label="<?= esc($__langNav === 'ur' ? 'فوری نیویگیشن' : 'Quick navigation') ?>">
    <a href="<?= base_url('student/dashboard') ?>" class="parent-bottomnav-fab" title="<?= esc($__langNav === 'ur' ? 'ڈیش بورڈ' : 'Home') ?>" aria-label="<?= esc($__langNav === 'ur' ? 'ڈیش بورڈ' : 'Dashboard home') ?>">
      <i class="fa fa-home" aria-hidden="true"></i>
    </a>
    <a href="<?= route_to('logout') ?>" class="nav-link parent-bottomnav-out" aria-label="<?= esc($__logoutLabel) ?>">
