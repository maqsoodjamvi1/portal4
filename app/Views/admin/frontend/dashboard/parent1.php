...
.quiz-card-parent {
  ...
  min-height: 360px;        /* allow card to grow instead of overlapping */
  height: auto;
}
...
if ($maxAttempts === 0) {
  ...
  <?php if (!empty($clsLabel)): ?>
    <div class="quiz-class-badge-wrap">
  ...
  <div class="parent-status-chip <?= $quizStatusClass ?>" data-bs-toggle="tooltip" title="Quiz status">
    <i class="fa <?= $quizStatusIcon ?>"></i> <?= $quizStatusLabel ?>
  </div>

  <!-- MAIN BLOCK: left info + right toppers -->
  <div class="quiz-main-block">
  <!-- LEFT SIDE: student + quiz info -->
  ...
  <div class="quiz-topper-block">
    <?php if (!empty($topScorers)): ?>
      <div class="quiz-small-info text-center">
        <i class="fa fa-trophy text-warning"></i> Top
      </div>
