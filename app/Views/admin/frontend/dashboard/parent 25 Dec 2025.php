...
.quiz-topper-block {
  flex: 0 0 72px;
  border-start: 1px dashed #e5e7eb;
  padding-left: 10px;
  display: flex;
  flex-direction: column;
  align-items: center;
...
<?php endif; ?>
  ...
  foreach ($quizzes as $quizRow) {
      ...
      // ✅ NEW: skip all closed quizzes (don’t show on dashboard at all)
      if ($quizStatus === 'closed') {
          continue;
      }

      $isClosed   = ($quizStatus === 'closed'); // now always false, but kept for clarity
...
if ($maxAttempts === 0) {
    ...
    $remainingAttempts = max($maxAttempts - $usedAttempts, 0);
              ...
              ?>
<?php 
$subjectName = trim($q['subject_name'] ?? ''); 
?>
  ...
  <!-- PREMIUM HEADER -->
   <div class="quiz-card-header">
    ...
    <div class="quiz-card-status <?= $quizStatusClass ?>" data-bs-toggle="tooltip" title="Quiz status">
  ...
  <a href="<?= base_url('student/battles?quiz_id='.$quizId) ?>" class="btn btn-warning btn-sm">
  ⚔️ Battle Mode
</a>
...
<style>
  /* Layout: left content + right toppers, no wrapping */
  .quiz-main-block {
    display: flex;
    flex-direction: row;
    align-items: stretch;
  ...
  .quiz-topper-block {
    flex: 0 0 64px;      /* fixed narrow right column */
    display: flex;
    flex-direction: column;
    align-items: center;
    margin-left: 8px;
  ...
  .quiz-top-avatars .avatar-wrap img {
    width: 100%;
    height: 100%;
    object-fit: cover;
  }
  ...
  /* Make sure on small screens it STILL stays side-by-side, not stacked */
...
<style>
  /* Right-side topper column */
  .quiz-topper-block {
    flex: 0 0 80px;              /* a bit wider so everything fits */
    display: flex;
    flex-direction: column;
    align-items: center;
    margin-left: 10px;
  ...
  .quiz-top-avatars .avatar-wrap {
    ...
    overflow: visible;           /* ✅ let badge show fully */
  }
  ...
  .quiz-top-avatars .pos-badge {
    position: absolute;
    bottom: -4px;                /* slightly outside bottom-right */
    right: -4px;
    min-width: 20px;
    height: 20px;
    line-height: 18px;
