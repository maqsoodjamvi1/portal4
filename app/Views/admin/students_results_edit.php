    ...
    <div class="col-lg-12">
      <div class="card sms-card card-primary card-outline card-tabs">
	    ...
	    <li class="nav-item"><a class="nav-link" href="<?= base_url('admin/students_results_card') ?>">View Results Cards</a></li>	
          ...
          <?php }else{ ?>
          <li class="nav-item"><a class="nav-link active" href="<?php echo base_url('admin/students_results/edit?id=') . $id; ?>"><?php echo $header;?></a></li>
          <?php } ?>
        </ul>
        <div class="card-body">
        <div class="tab-content">
		<?php
			echo form_open( base_url('admin/students-results/save'), 'role="form" id="user-edit-form"');
...
<script>
 $("#cls_sec_id").change(function(){		
		...
		$.ajax({
        	url: '/admin/students-results/get-students',
          type: "POST",
 ...
 $("#campus_id").change(function(){
  ...
  $("#class_id").change(function(){
        var class_id = $('#class_id').val();
         $.ajax({
            url: '/admin/ajax/selectsubjectsforresult',
            type: "POST",
