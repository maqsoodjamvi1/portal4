  ...
  <div class="chalanwrapper">
    ...
    <div class="feeinfo">
      <div class="chalanrows">
        Chalan#: <?= esc($student_info['chalan_id'] ?? 'N/A') ?>
        <span style="float:right; margin-right:10px;">Family#: <?= esc($student_info['parent_id'] ?? '') ?></span>
    ...
    <table width="98%" border="1" class="feetable">
      ...
      <?php foreach ($student_info['student_fee'] ?? [] as $fee_info): ?>
        ...
        <?php else: ?>
          <?php $arialSum += $net; ?>
        <?php endif; ?>
        <?php if ($fee_info['is_monthly_fee'] == 1): $total -= $fee_info['discount']; endif; ?>
        <?php $nCount++; ?>
      <?php endforeach; ?>
