...
<?php
// Define once for this request so we can use monthSymbol($m) in the table
if (!function_exists('monthSymbol')) {
    function monthSymbol(array $m): string {
        switch ($m['status'] ?? 'unpaid') {
            case 'paid_on_time': return '✓'; // paid on/before due date
            case 'paid_late':    return '⚠'; // paid after due date
...
<style>
  .last12-wrap{ margin-top:6px; }
  .last12-table{
    width:100%; border-collapse:collapse; table-layout:fixed;
    border:1px solid #000; direction:ltr; /* avoid RTL flipping months */
  }
  .last12-table th, .last12-table td{
    border:1px solid #000; text-align:center; vertical-align:middle;
    padding:4px; font-size:13px; line-height:1.2; height:28px;
    white-space:nowrap; overflow:hidden; text-overflow:ellipsis;
  }
  .last12-table th{ font-weight:700; }
  .legend{ margin-top:4px; font-size:12px; direction:ltr; }
  .legend span{ margin-right:10px; }
  @media print {
    .last12-table th, .last12-table td{ font-size:12px; height:24px; padding:3px; }
  }
</style>
...
<div class="info-row grid-2">
  ...
  <td><?= esc(number_format($payableMonthly, 0)) ?>/-</td>
  <td></td>
</tr>
<tr class="fee-summary-mini-row">
  <td>Other fee (unpaid)</td>
  <td><?= esc(number_format($payableOther, 0)) ?>/-</td>
  <td></td>
</tr>
<tr class="total-row">
  <td><strong>Total payable</strong></td>
  <td><strong><?= esc(number_format($payableTotal, 0)) ?>/-</strong></td>
  <td></td>
</tr>
...
<?php
// Expect: $student_info['unpaid_last12'] from controller (12 items)
$last12 = $student_info['unpaid_last12'] ?? [];

// Fallback: synthesize empty 12 months (oldest->newest)
if (count($last12) !== 12) {
  $last12 = [];
...
// Split into 6 + 6 (oldest->newest)
$first6 = array_slice($last12, 0, 6);
$last6  = array_slice($last12, 6, 6);
?>

<div class="last12-wrap">
  <table class="last12-table">
    <colgroup><col span="6" style="width: calc(100% / 6);"></colgroup>
    <thead>
      <tr>
        <?php foreach ($first6 as $m): ?>
          <th><?= esc($m['label']) ?></th>
        <?php endforeach; ?>
      </tr>
    </thead>
  ...
  <table class="last12-table" style="margin-top:4px;">
    <colgroup><col span="6" style="width: calc(100% / 6);"></colgroup>
    <thead>
      <tr>
        <?php foreach ($last6 as $m): ?>
          <th><?= esc($m['label']) ?></th>
        <?php endforeach; ?>
      </tr>
    </thead>
