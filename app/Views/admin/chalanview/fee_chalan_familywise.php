...
<div id="fee-chalan-wrapper">
    ...
    $('#student_id').select2({
      ...
      width: '100%',
      dropdownParent: $('#student_id').closest('.modal, .card, .content-wrapper') // picks the nearest wrapper
    });
    ...
    loadFeeChalans(studentId, parentId);
  });


  function loadFeeChalans(studentId, parentId) {
    $('#fee-chalan-wrapper').html('<div class="p-3 text-muted">Loading…</div>');
    $.ajax({
      url: '<?= base_url('admin/fee-chalan-pay/get-chalans'); ?>',
      type: 'POST',
      data: {
      ...
      success: function (html) {
        $('#fee-chalan-wrapper').html(html);
      },
      error: function (xhr) {
        $('#fee-chalan-wrapper').html(
          '<div class="alert alert-danger mb-0">Failed to load fee chalans.</div>'
        );
