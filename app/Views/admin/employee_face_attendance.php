...
<div class="card-header">
    ...
    <h3 class="card-title mb-0">Employee Face Scanner</h3>

    <p class="text-muted small mb-0">Admin captures each employee&apos;s face. First scan today = check-in; second = check-out.</p>

    <p class="text-muted small mb-0">

        <a href="<?= base_url('admin/employee-face-management') ?>">Enroll faces</a>

        &middot;

        <a href="<?= base_url('admin/employees_attendance/add') ?>">Manual attendance</a>
...
<div class="card-body text-center">
    ...
    <div class="mb-2">
        ...
        <button type="button" id="btnMarkAttendance" class="btn btn-success btn-lg px-4">
            ...
            <i class="fa fa-sign-in-alt"></i> Scan (check-in / check-out)
...
(function () {
    ...
    function humanReadableCameraError(err) {
        ...
        if (err.name === 'NotReadableError' || err.name === 'TrackStartError') {
            ...
            return 'The camera could not be started. Close other tabs using the camera, then click Retry camera.';
    ...
    function startCamera() {
        ...
        return getUserMediaWithFallbacks().then(function (stream) {
            ...
            setStatus('Camera ready. Position the employee and tap Scan.', false);
    ...
    btn.addEventListener('click', function () {
        ...
        captureOptimizedBlob(function (blob) {
            ...
            $.ajax({
                ...
                url: "<?= base_url('admin/employee-face-attendance/mark') ?>",
                ...
                if (res.success) {
                    ...
                    setStatus(res.msg || 'Done.', false);

                    var parts = [];

                    if (res.employee_name) parts.push(res.employee_name);

                    if (res.action) parts.push(res.action === 'checkin' ? 'Check-in' : 'Check-out');

                    if (res.status) parts.push('Status: ' + res.status);

                    if (typeof res.similarity === 'number') parts.push('Match: ' + res.similarity + '%');

                    setMeta(parts.join(' · '));

                } else {

                    setStatus(res.msg || 'Could not process scan.', true);
