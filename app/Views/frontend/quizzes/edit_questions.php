        ...
        <form action="<?= site_url('admin/quizzes/update-questions/' . $quizId) ?>" 
            ...
            <div class="card-body">
                ...
                <div class="form-row mb-4">
                        ...
                        <select name="class_id" id="edit_class_id" class="form-control" required>
                            ...
                            <?php foreach ($classes as $c): ?>
                                <option value="<?= $c->class_id ?>" 
                                    <?= ($quizInfo->class_id ?? 0) == $c->class_id ? 'selected' : '' ?>>
                                    <?= esc($c->class_name) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <select name="subject_id" id="edit_subject_id" class="form-control" required>
                            ...
                            <?php foreach ($subjects as $s): ?>
                                <option value="<?= $s->sid ?>" 
                                    <?= ($quizInfo->subject_id ?? 0) == $s->sid ? 'selected' : '' ?>>
                                    <?= esc($s->subject_name) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="form-group col-md-4">
                        <label for="edit_topic_id">Topic</label>
...
$(document).ready(function() {
    ...
    // Topic dropdown based on subject
    $('#edit_subject_id').change(function() {
        ...
        $.get('<?= site_url('admin/question-bank/topics') ?>?class_id=' + classId + '&subject_id=' + subjectId, function(data) {
            const topics = Array.isArray(data) ? data : (data.topics || []);
            const select = $('#edit_topic_id');
            select.html('<option value="">-- Select Topic --</option>');
            
            topics.forEach(function(t) {
                select.append(new Option(t.topic_name, t.id));
            });
        }).fail(function() {
            $('#edit_topic_id').html('<option value="">-- Select Topic --</option>');
        });
