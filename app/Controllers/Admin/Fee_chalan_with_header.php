...
class Fee_chalan_with_header extends MY_Controller {
...
public function index()
{
	...
	$this->load->view('fee_chalan_with_header', $this->template_data);
}
...
function data(){
	...
	$fine_after_due_date = $this->input->get('fine_after_due_date');
	...
	$late_fee_fine = $this->db->query('SELECT late_fee_fine,fine_type from campus where campus_id='.$campus_id)->row();
	...
	$this->db->where($where);
	$this->db->order_by("chalan_id", "DESC");
	...
	foreach($fee_chalan as $chalanvalue){
		...
		'chalan_header' => $systemInfo->chalan_header,
		'location' => $location,	
		'bank_name' => $bank_name,	
		...
		'fee_fine' =>$fee_fine,
		'late_fee_fine' => $late_fee_fine->late_fee_fine,
		'fine_type' => $late_fee_fine->fine_type
		);
