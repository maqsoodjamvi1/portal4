...
class Worksheet_info extends MY_Controller {
	...
	*/
	...
	{
		$this->load->view('worksheet_info', $this->template_data);
	}
	...
	function add(){
		...
		$this->load->view('worksheet_info_edit', $this->template_data);
    ...
    foreach ($worksheetinfo as $key => $value) { 
      ...
      $path .='<tr><td>	
             ...
             <input type="text" id="slugme'.$i.'" class="form-control" placeholder="Title" name="title'.$i.'" value="'.$value->doc_title.'">
          	...
          	$path .='<textarea rows="3" placeholder="Description"  name="text'.$i.'" class="form-control">'.$value->doc_description.'</textarea></td>';
      '</tr>';	
         $i++;  
       } 
        }else{ 
