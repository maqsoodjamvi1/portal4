...
public function AdvFee()
{
	$user_id    = $this->session->userdata('member_userid');
	$date       = date('Y-m-d H:i:s');
	$paid_date  = date('Y-m-d');
	$campusid   = $this->session->userdata('member_campusid');
	$issue_date = date('Y-m-d');         // New: current issue date
	$fee_month  = date('Y-m');           // New: fee_month in 'YYYY-MM' format
	$discount   = 0;                     // New: default discount value
	...
	foreach ($student_ids as $key => $value) {
		...
		$studentFeeInfo = $this->db->query("
			SELECT * FROM fee_chalan 
			WHERE student_id = $value 
			  AND fee_type_id = (
				SELECT fee_type_id FROM fee_type 
				WHERE s_flag = 1 AND fee_type_id = 194
			  )
		")->row();
		...
		if ($studentFeeInfo) {
			...
			$data = array(
				...
				'status'       => 'paid',
				'paid_date'    => $paid_date,
				'issue_date'   => $issue_date,   // Added
				'fee_month'    => $fee_month,    // Added
				'created_date' => $date,
				'user_id'      => $user_id
			);
			$this->db->insert('fee_chalan', $data);
