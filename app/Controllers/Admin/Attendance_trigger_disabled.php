...
class Attendance_trigger extends MY_Controller {
	...
	public function index()
	{
		...
		foreach ($campusinfo as $key => $campus) {
			...
			foreach ($sectionsinfo as $key => $section) {
				...
				$schooltimingsinfo = $this->db->query("SELECT *,(checkout_timing-checkin_timing) AS duration FROM school_timings WHERE
				 cls_sec_id =".$section->cls_sec_id." AND dayname ='".$day."'  AND type_id = (SELECT type_id FROM school_timing_types WHERE STATUS=1 )")->row();
