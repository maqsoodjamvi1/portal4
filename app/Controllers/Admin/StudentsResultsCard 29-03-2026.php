...
class StudentsResultsCard extends BaseController
{
    ...
    //             return $this->asHtml('<div class="alert alert-warning">No students found for this class/section.</div>');
    //         }
...
private function fetchStudentClassRows($sessionId, $clsSecId, $campusId, $statusFilter = null)
{
    ...
    return $builder->orderBy('s.reg_no', 'ASC')
                   ->get()
                   ->getResult(); // Changed from getResultArray() to getResult()
}
public function data()
{
      try {
        ...
        // 2) Fetch students of selected class-section, campus & session WITH status filter
        $studentClass = $this->fetchStudentClassRows(
            (int)$ctx['session_id'],
            (int)$ctx['cls_sec_id'],
            (int)$ctx['campus_id'],
            $statusFilter // Pass the filter here!
        );
        // Optional quick debug
        if (empty($studentClass) && ($this->request->getPost('debug') === '1')) {
            $counts = [
        ...
        // 6) NO NEED for additional status filtering here - already done in the query!
        // The status filter was already applied when fetching $studentClass
    ...
    {
        // examids normalization (CSV or array)
        ...
        // boolean flags with aliases & common typos
        $useShortName   = $this->getFlag($request, ['useShortName','use_short_name']);
        $showMarks      = $this->getFlag($request, ['showMarks','show_marks','marks'], true); // default ON
        $showPercentage = $this->getFlag($request, [
            ...
            'showpersentage','show_persentage','persentage' // common typos
        ], false);
        ...
        // Safety: ensure at least marks so the table never collapses to “Subject” only
    ...
    # =========================================================
    # Sub-fn: Fetch students of selected class/section
    # =========================================================
 
    # =========================================================
    # Sub-fn: Fetch exam info
    # =========================================================
