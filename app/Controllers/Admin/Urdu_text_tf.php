...
class Urdu_text_tf extends MY_Controller {
	...
	*/
	...
	public function index()
	{
		...
		$this->load->view('urdu_text_tf', $this->template_data);
	}
...
function add(){
	...
	if(isset($topic_id)){
		...
		$this->load->view('urdu_text_tf_edit', $this->template_data);
}
...
function save(){
	...
	for($i=0; $i < count($optionscount); $i++){
		...
		$data2 = array(
				...
				'topic_skills_id' => trim($this->input->post('topic_skill_id')),
				'content_type_id' => trim($this->input->post('content_type_id')),
				'question_id' => trim($new_question_id),
				'diff_level' => 0,
				'temp_id' => 0,
				'quiz_Learning' => 0
			);
	...
	$this->output->set_output(json_encode(array('success' => TRUE, 'msg' => 'Add Urdu Question Success')));
