...
class Fee_chalan_single_copy extends MY_Controller {
...
public function index()
{
  ...
  $this->load->view('fee_chalan_single_copy', $this->template_data);
}
...
function data(){
	...
	if($cls_sec_id){
		$result = $this->db->query('SELECT t1.cls_sec_id,t2.student_id, t2.campus_id,t2.reg_no,t2.first_name,t2.last_name,t2.parent_id FROM student_class t1, students t2 WHERE t1.student_id = t2.student_id  and t2.campus_id='.$campus_id.' AND t1.cls_sec_id='.$cls_sec_id.' order by t1.cls_sec_id asc')->result();
	}else{	
		$result = $this->db->query('SELECT t1.cls_sec_id,t2.student_id, t2.campus_id,t2.reg_no,t2.first_name,t2.last_name,t2.parent_id FROM student_class t1, students t2 WHERE t1.student_id = t2.student_id  and t2.campus_id='.$campus_id.' order by t1.cls_sec_id asc')->result(); 
	}
