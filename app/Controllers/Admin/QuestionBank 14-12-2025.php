...
public function save()
{
    ...
    // Optional: log raw incoming data once (for debugging)
    ...
    foreach ($questions as $idx => $q) {
        ...
        // Basic validation
        if ($classId <= 0 || $subjectId <= 0 || $topicId <= 0 || $question === '') {
            $skipped++;
            log_message(
                'warning',
                'QB save() skipping question index ' . $idx . ' due to missing class/subject/topic/question.'
            );
            continue;
        }
...
$data['is_drag'] = (isset($q['is_drag']) && $q['is_drag'] == '1') ? 1 : 0;
        // Ensure we always define these keys (even if null)
        $data['option_a']     = null;
        $data['option_b']     = null;
        $data['option_c']     = null;
        $data['option_d']     = null;
    ...
    elseif ($type === 'mcq_multi') {
        ...
        // Save options A–D (in both columns AND JSON)
        $opts = [
            'A' => $q['option_a'] ?? '',
            'B' => $q['option_b'] ?? '',
            'C' => $q['option_c'] ?? '',
        ...
        // ✅ also fill the normal columns so they are not NULL
        $data['option_a'] = $opts['A'];
        $data['option_b'] = $opts['B'];
        $data['option_c'] = $opts['C'];
        $data['option_d'] = $opts['D'];

        // Multiple correct options from checkboxes
        ...
        // Store MCQ-MULTI meta in JSON
        $data['options_json']   = json_encode($json, JSON_UNESCAPED_UNICODE);
        $data['correct_option'] = null;  // single-letter is not used for mcq_multi
        $data['answer_text']    = null;  // not used either
    }
        ...
        //     $data['options_json']   = json_encode($json, JSON_UNESCAPED_UNICODE);
        //     $data['correct_option'] = null;  // single-letter is not used
        // }
...
// exit;
            ...
            if (! $ok) {
                // Insert returned false – log exact error
                $dbError = $this->db->error();
                log_message(
                    'error',
                    'QB save() insert failed for index ' . $idx . ' : '
                    . json_encode($dbError) . ' | data=' . json_encode($data)
                );
                $skipped++;
                continue;
            }
            ...
            // If an exception occurs, log full context
            log_message(
                'error',
    ...
    // Final message: how many actually saved vs total attempts
    if ($savedCount > 0) {
        return redirect()
