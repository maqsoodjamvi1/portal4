...
public function add()
{
    ...
    // For ALL users (teachers, role 5, admin), show all sections/subjects
    // No restriction - everyone sees all class sections
...
public function selectSectionSubjectbySection()
{
    ...
    $rolesRaw   = (array) currentUserRoles();
    $roles      = array_map('intval', $rolesRaw);
    $isTeacher  = in_array(5, $roles, true);
...
.sr-name{flex:1 1 auto;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-weight:600;color:#0f172a}
...
@media (max-width:768px){
  ...
  .td-left{display:flex;align-items:center;gap:10px;min-width:0;flex:1 1 auto}
  .sr-idx{width:26px;height:26px;font-size:.8rem;flex:0 0 26px}
  .sr-avatar{width:36px;height:36px;flex:0 0 36px}
  .sr-name{font-size:.95rem}
  .td-right{flex:0 0 auto}
...
.ssr-note{font-size:.8rem;color:#475569}
    ...
    $studentsList .= '<div class="ssr-toolbar">
      <div class="ssr-legend">
        <span class="badge badge-light border">Autosave</span>
    ...
    foreach ($students as $s) {
        ...
        $badge = $isAbsent
            ...
            : ($attStatus ? '<span class="badge badge-success ml-1">Present</span>' : '');

        // One row: left (idx+avatar+name) | right (marks)
        $studentsList .= '<tr>
          <td class="td-left" colspan="2">
            <span class="sr-idx">'.($i++).'</span>
            '.$avatarHtml.'
            <span class="sr-name">'.htmlspecialchars($studentName).'</span>'.$badge.'
