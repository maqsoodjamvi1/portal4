    ...
    protected function getParentQuizzes(int $parentId): array
{
    ...
    $sql = "
        SELECT
            ...
            -- FIX 1: Get LAST attempt score (not best)
            (
                SELECT qa2.score_obtained
                FROM quiz_attempts qa2
                WHERE qa2.quiz_id = q.quiz_id
                    AND qa2.student_id = s.student_id
                    AND qa2.status = 'submitted'
                ORDER BY qa2.attempt_id DESC
                LIMIT 1
    ...
    foreach ($rows as &$row) {
        ...
        $questionsCount = (int)($row['questions_count'] ?? 0);
        ...
        // ---- FIX 2: Calculate last attempt percentage ----
        $lastAttemptScore = (float)($row['last_attempt_score'] ?? 0);
        if ($questionsCount > 0 && $lastAttemptScore > 0) {
            $row['last_attempt_percentage'] = ($lastAttemptScore / $questionsCount) * 100;
        } else {
            $row['last_attempt_percentage'] = 0;
        }

        // ---- Top scorers with percentage calculation ----
        if ($quizId > 0 && !isset($topCache[$quizId])) {
            $topCache[$quizId] = $db->query("
                SELECT
                    ...
                    (SELECT questions_count FROM quizzes WHERE quiz_id = ?) as total_questions
                FROM quiz_attempts qa
                JOIN students s ON s.student_id = qa.student_id
                WHERE qa.quiz_id = ?
                    AND qa.status = 'submitted'
                    AND qa.score_obtained IS NOT NULL
                    -- Get only latest attempt for each student
                    AND qa.attempt_id = (
        ...
        if (!empty($rawTop)) {
            ...
            foreach ($rawTop as $ts) {
                ...
                $score = (float)($ts['score_obtained'] ?? 0);
                $totalQs = (int)($ts['total_questions'] ?? $questionsCount);
                $percentage = $totalQs > 0 ? ($score / $totalQs) * 100 : 0;

                $topScorers[] = [
                    'student_id'     => (int)$ts['student_id'],
                    'student_name'   => $ts['student_name'],
                    'score'          => $score, // Changed from score_obtained to score
