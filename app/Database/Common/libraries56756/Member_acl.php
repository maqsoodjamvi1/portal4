...
/* 		function ACL($userID = '')
		...
		function getUserRoles()
		{
			...
			$userinfo = $this->CI->db->query('select * from users WHERE id='.$userid)->row();

			$campusBillinfo = $this->CI->db->query('select * from campus_bills WHERE campus_id='.$userinfo->campus_id.' AND status=1')->row();
			...
			if($userid){
				if(!$resp = $this->CI->cache->get('user_roles_' . $userid)){
					...
					//$strSQL = "SELECT id from roles WHERE role_name_id IN (SELECT role_name_id FROM user_roles WHERE userID = " . floatval($this->userID) . ") AND plan_id = ".$campusBillinfo->plan_id;
					$strSQL = "SELECT id FROM roles WHERE role_name_id IN (SELECT roleid FROM user_roles WHERE userID = " . floatval($this->userID) . ") AND plan_id = ".$campusBillinfo->plan_id;
