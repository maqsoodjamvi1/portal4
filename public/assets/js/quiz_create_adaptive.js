...
(function ($) {
  ...
  function renderPills() {
    ...
    keys.forEach(function (no) {
      ...
      $btn.html('Level ' + no + ' <span class="badge ' + pillClass + ' ms-1">' + cnt + '</span>');
  ...
  function renderEditor() {
    ...
    if (!(L.question_ids || []).length) {
      ...
      $chips.append('<span class="text-muted me-1">' + L.question_ids.length + ' question(s):</span>');

      L.question_ids.forEach(function (id) {

        $chips.append('<span class="badge text-bg-primary me-1 mb-1">#' + id + '</span>');
      ...
      if (lvl) {
        ...
        if (!$row.find('.qb-level-badge').length) {
          ...
          $row.find('.qb-q-item-meta').append(
            ...
            '<span class="badge text-bg-primary qb-level-badge ms-1">' + badgeText + '</span>'
