(function () {
  ...
  function syncAnswerFieldName(type) {
    ...
    var desc = el('qb_pe_answer_descriptive');
    ...
    if (desc) desc.removeAttribute('name');
    ...
    } else if (type === 'descriptive' && desc) {

      desc.setAttribute('name', 'questions[0][answer_text]');
  ...
  function toggleMediaMode(media) {
    ...
    var imageNote = document.querySelector('#qbProofEditModal .qb-pe-image-note');

    if (textWrap) textWrap.classList.toggle('d-none', media === 'image');

    if (imageNote) imageNote.classList.toggle('d-none', media !== 'image');
  ...
  function fillForm(q) {
    ...
    el('qb_pe_topic_id').value = q.topic_id || '';
    ...
    if (el('qb_pe_answer_descriptive')) el('qb_pe_answer_descriptive').value = ans;
