...
(function (window, $) {
      ...
      (cls.subjects || []).forEach(function (subj) {
        (subj.topics || []).forEach(function (t) {
          ...
          topicMeta[k] = {
            ...
            board_publishers: t.board_publishers || [],
          };
        });
      });
    });
  ...
  function updateSelectedPanel() {
    ...
    keys.forEach(function (k) {
      ...
      html +=
        '<span class="badge text-bg-primary qb-chip me-1 mb-1">' +
        escHtml(label) +
  ...
  function renderTopicList() {
    ...
    topics.forEach(function (t) {
      ...
      let boardHtml = '';
      (t.board_publishers || []).forEach(function (bp) {
        const lbl = bp.short_code || bp.name || '';
        if (lbl) {
          boardHtml += '<span class="badge text-bg-secondary ms-1">' + escHtml(lbl) + '</span>';
        }
      });
      html +=
        ...
        boardHtml +
        '</span><span class="badge text-bg-info">' +
        (t.question_count || 0) +
        '</span></label>';
    });
  ...
  window.QuestionPaperQbBrowser = {
    ...
    focusClassSubject: function (classId, subjectId) {
      if (!qbTree.length) {
        return;
      }
      activeClassId = classId;
