(function () {
  ...
  function buildCardHtml(q, idx) {
    ...
    return (
      ...
      '<div><input type="checkbox" class="js-select me-2" ' +
  ...
  function setGenerateLoading(active) {
    ...
    const topicDesc = document.getElementById('topic_description');
    const btnSaveTopicDesc = document.getElementById('btnSaveTopicDesc');
    const btnAddTopic = document.getElementById('btnAddTopic');
    if (topicDesc) {
      topicDesc.disabled = active;
    }
    if (btnSaveTopicDesc) {
      btnSaveTopicDesc.disabled = active;
    }
    if (btnAddTopic) {
      btnAddTopic.disabled = active;
    }
    if (!active) {
  ...
  btnGenerate.addEventListener('click', async () => {
    ...
    generateStatus.textContent = '';
    generateStatus.className = 'text-muted ms-2';
    ...
    const topicDesc = typeof window.getQbAiTopicDescription === 'function'
      ? window.getQbAiTopicDescription()
      : (document.getElementById('topic_description')?.value || '').trim();
    body.append('topic_description', topicDesc);
    ...
    try {
      ...
      generateStatus.className = 'text-success ms-2';
